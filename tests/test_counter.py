# source: https://github.com/Ackee-Blockchain/woke/blob/main/examples/counter/tests/test_counter.py
...
def tx_callback(tx: TransactionAbc):
    ...
    for event in tx.raw_events:
        if len(event.topics) > 0 and event.topics[0] == Gateway.Relay.selector:
            ...
            data, destination_chain = Abi.decode(
                ["bytes", "string"], event.data)
