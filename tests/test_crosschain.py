# source: https://github.com/Ackee-Blockchain/woke/blob/main/examples/counter/tests/test_crosschain.py
import logging
...
class CrosschainTest(FuzzTest):
    ...
    def pre_sequence(self) -> None:
        ...
        self.gw1 = Gateway.deploy(
            from_=random_account(chain=chain1), chain=chain1)
        self.gw2 = Gateway.deploy(
            from_=random_account(chain=chain2), chain=chain2)
        self.c1 = Counter.deploy(
            from_=random_account(chain=chain1), chain=chain1)
        self.c2 = Counter.deploy(
            from_=random_account(chain=chain2), chain=chain2)
    ...
    def post_flow(self, flow: Callable) -> None:
        ...
        if self.first_chain:
            ...
            dest_chain = "chain2"
            logger.info(
                f"Relaying count after flow {flow.__name__}: {counter.count()} from chain 1 to chain 2")
        else:
            gw = self.gw2
            other_gw = self.gw1
            counter = self.c2
            ...
            dest_chain = "chain1"
            logger.info(
                f"Relaying count after flow {flow.__name__}: {counter.count()} from chain 2 to chain 1")

        # encode the call to set the count on the other chain
        payload = Abi.encode_call(Counter.setCount, [counter.count()])
        tx = gw.relay(other_counter.address, payload, dest_chain,
                      from_=random_account(chain=gw.chain))

        # relay the data (command) based on the events emitted by the gateway
        for event in tx.events:
            if isinstance(event, Gateway.Relay):
                other_gw.execute(event.target, event.data,
                                 from_=other_gw.owner())
